// k6 run media/tambah_foto_pengiriman_ekspedisi_picked_up.js

import http from 'k6/http';
import { check, sleep } from 'k6';

// 1️⃣ Baca file sekali di awal (foto picked-up ekspedisi)
const fileBytes = open('foto/ekspedisi_picked_up.jpg', 'b');
...
export default function () {
  ...
  const presignedRes = http.put(
    "http://localhost:8080/kurir/media/tambah-foto-pengiriman-ekspedisi-picked-up",
    payload,
    {
  ...
  check(uploadRes, {
    "upload picked-up ekspedisi success": (r) => r.status === 200 || r.status === 204,
  });
