// k6 run media/tambah_foto_pengiriman_picked_up.js

import http from 'k6/http';
import { check, sleep } from 'k6';

// 1️⃣ Baca file sekali di awal (foto pengiriman)
const fileBytes = open('foto/picked_up.jpg', 'b');
...
export default function () {
  ...
  const payload = JSON.stringify({
    ...
    id_pengiriman: 101, // ⬅️ sesuaikan ID pengiriman
    ekstensi: "jpg",
  });

  const presignedRes = http.put(
    "http://localhost:8080/kurir/media/tambah-foto-pengiriman-non-ekspedisi-picked-up",
    payload,
    {
  ...
  check(uploadRes, {
    "upload picked-up success": (r) => r.status === 200 || r.status === 204,
  });
